...
ある個人プロジェクトをWordPressからJamstackに移行する際、私はContentful・CraftCMS・Strapi・WordPressのヘッドレス使いなど、様々なヘッドレスCMSを試しました。しかし、数ヶ月かけても自分の要望に合致するものが見つからず。

そんな時に出会ったのがこのVS Codeの拡張機能、[Front Matter CMS](https://frontmatter.codes/)です。ローカルで複雑な構造のコンテンツ群も管理出来る素晴らしさ。
...
その点、Front Matter CMSを使えば、すべて自分で設計・管理が出来、どんなに記事や画像が増えても料金の心配はありません。

また、オフラインでコンテンツ管理が可能なので、インターネット接続によるラグもありません。即座にファイル作成・保存が出来ます。

### Front Matter CMSのデメリット

- 技術者以外への導入の敷居が高い
- リッチなテキストエディタは使えない（Markdownのみ）
- VS Code以外では動かない
- 英語のみ
...
更に、GitのインストールなどのPCの設定も必須となるため、オンラインで記事管理が出来るContentful等のヘッドレスCMSに比べて、技術者以外には非常に敷居が高くなります。

また、現時点では多言語でのローカライズはされていないため、項目メニュー等自分で設定出来る部分を除いて、Front Matter CMS本体は英語表示になります。

## Front Matter CMSで出来ること

Front Matter CMSでは、

- CMS設計: `frontmatter.json`
- 画像のメタデータ・タクソノミー: `.frontmatter/database/` フォルダ内のjsonファイル

で管理します。

その他、カテゴリーやメニュー等を任意のフォルダにjsonファイルで管理し、frontmatter.jsonで読み込むこともあります。
...
![Front Matter CMS {925x501}](../../../images/frontmattercms01.ja.png)
...
MarkdownコンテンツではGutenbergのようなリッチな表現はできないものの、例えばアラートボックスなど、予め「よく使う」スニペットデータを保存しておけます。

![Front Matter CMS {925x501}](../../../images/frontmattercms07.ja.png)
...
## Front Matter CMSで出来ないこと

### リピーターフィールドは作れない

WordPressのAdvanced Custom Fieldsプラグインのような、動的に追加・削除が出来るリピーターフィールドを作る機能は、現時点ではFront Matter CMSにはありません。

### リッチテキストエディタは使えない

「デメリット」にも書きましたが、リッチテキストエディタ（WYSIWYGエディタ）等は使えません。**Front Matter CMSは、MarkdownコンテンツのためのCMS**です。
...
Jamstack + Markdownでコンテンツ管理を考えているなら、是非一度Front Matter CMSを試してみて下さい。設計によっては、かなり複雑なメタデータ構造を持つMarkdownのコンテンツ管理も出来る、素晴らしいヘッドレスCMSです。
...
また、もし今後、VS Code Onlineがこのような拡張機能に対応出来れば、オンラインでの共同コンテンツ管理が楽になるので、クライアント向け納品物にも使えるかもしれないかなとも思います。
